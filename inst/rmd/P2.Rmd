...
if (params$KPnames == TRUE){
  demographics    <- "chords_demographics"
  encounters      <- "chords_encounters"
  census_location <- "chords_census_location"
  diagnoses       <- "chords_diagnoses"
  vital_signs     <- "chords_vital_signs"
  lab_results     <- "chords_lab_results"
  procedures      <- "chords_procedures"
} else {
...
tab1_sh <- odbc::dbGetQuery(con, "select count(*) as nrows, 
  ...
  count(distinct enc_id) as nencts,
	...
	from chords_social_history")

tab1_ps <- odbc::dbGetQuery(con, "select count(*) as nrows
	from chords_provider_specialty")

...
##Table 1A. Surgeries by Year and Location
...
tab1a <- odbc::dbGetQuery(con, 
  paste0("select floor(DATEDIFF(day, b.birth_date, a.procdate)/365.25) as age_px,
          PX as code,
          YEAR(procdate) as year
          from procedures as a left join (select distinct person_id, birth_date from demographics) as b on a.person_id = b.person_id
            where (a.procdate between '", start_dt, "' AND '", end_dt,"')
              AND floor(DATEDIFF(day, b.birth_date, a.procdate)/365.25) between 10 and 26
          ORDER BY YEAR(procdate), PX"))

tab1a_tot <- odbc::dbGetQuery(con, 
  paste0("select count(*) nprocs,
          YEAR(a.procdate) as year
          from procedures as a left join (select distinct person_id, birth_date from demographics) as b on a.person_id = b.person_id
            where (procdate between '", start_dt, "' AND '", end_dt,"')
              AND floor(DATEDIFF(day, b.birth_date, a.procdate)/365.25) between 10 and 26
          GROUP BY YEAR(a.procdate)
          ORDER BY YEAR(a.procdate)")) %>% 
  as.tbl(.) %>% 
  dplyr::


tab1a1 <- tab1a %>% 
  dplyr::filter(code %in% codes2$code) %>% 
  left_join(., codes2, by = "code") %>% 
  dplyr::select(year, surg_type) 

tab1a1<-  as.data.frame(table(tab1a1$year, tab1a1$surg_type)) %>% 
  dplyr::rename(year = Var1, surg_type = Var2)

if(!("pectus" %in% tab1a1$surg_type)){
  pectus_null <- data.frame(year = tab1a1$year,
                               surg_type = rep("pectus", length(tab1a1$surg_type)),
                               Freq = rep(0, length(tab1a1$Freq))
  )
  tab1a1 <- dplyr::bind_rows(tab1a1, pectus_null)
}

tab1a2 <- dplyr::left_join(tab1a1, as.tbl(tab1a_tot), by = "year")

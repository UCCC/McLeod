...
I need to add in count of unique combination of procedures and mean/median length of stay
...
tab3a2_prov <- tab3a_prov %>% 
  ...
  mutate(
    cohort = if_else(ENC_ID %in% tab1b2$ENC_ID, 1, 0)
  ) %>% 
  summarise(n_procs = sum(cohort), spine = sum(spine), pectus = sum(pectus)) %>% 
  dplyr::select(Year = px_date, Provider_Specialty = Description, Cohort_PX = n_procs, spine, pectus)
...
knitr::kable(tab3a2_prov, col.names = c("Year", "Provider Specialty", "Cohort Procedures", "Total Spine Procedures", "Total Pectus Procedures"))

```

## Table Pack 3C: Surgeon Demographics per Facility
...
tab3b <- tab3a_prov %>% 
  ...
  mutate(
    ...
    cohort_proc = if_else(ENC_ID %in% tab1b2$ENC_ID, 1, 0),
    sum_cohort = sum(cohort_proc),
  ...
  ungroup(.) %>% 
  select(PERFORMINGPROVIDER, sum_spine, sum_pectus, sum_cohort, SPECIALTY, PROVIDER_BIRTH_YEAR, p_race, p_hisp, YEAR_GRADUATED, max_proc) 

```

```{r tab3b_out, results="asis"}

knitr::kable(tab3b, col.names = c("Physician ID", "Count Spine PXs", "Count Pectus PXs", "Count Cohort PXs", "Physician Specialty", "Physician Birth Year", "Physician Year Graduated", "Physician Race", "Physician Ethnicity", "Total Volume"))

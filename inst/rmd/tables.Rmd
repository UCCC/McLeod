...
GROUP BY z.ENC_ID,
        ...
        d.RX_REFILLS,
        d.RX_DAYS_SUPPLY,
        d.RX_FREQUENCY, d.GENERIC_MED_NAME, z.PROCDATE, YEAR(z.ADATE)"
)
...
tab4_test2 <- as.data.frame(table(tab4a2_2$ProcedureType, tab4a2_2$PERSON_ID, tab4a2_2$GENERIC_MED_NAME))


tab4_sub <- tab4_test2 %>% 
  arrange(Var3) 

tab4_wide <- spread(tab4_sub, key = Var3, value = Freq)
...
tab4_test3 <- as.data.frame(table(tab4a2_2$GENERIC_MED_NAME, tab4a2_2$RX_DAYS_SUPPLY))

tab4_wide <- spread(tab4_test3, key = Var1, value = Freq) %>% 
  dplyr::rename(Days = Var2)

tab4_plot <- ggplot(data = tab4_test3, aes(x = Var2, y = Freq, group = as.factor(Var1))) +
  geom_line(aes(color = as.factor(Var1)))

p <- plotly::ggplotly(tab4_plot)
plotly::api_create(p, filenaame = "dist-rx-day-supply")

lapply(2:ncol(tab4_wide), function(x) 
  ggplot(data = tab4_wide, aes(x = Var2, y = fig1_wide[[x]])) +
         geom_line() +
...
tab4_test4 <- as.data.frame(table(tab4a2_2$PERSON_ID, tab4a2_2$GENERIC_MED_NAME, tab4a2_2$RX_DAYS_SUPPLY)) %>% 
  arrange(Var1) %>% 
  dplyr::filter(Freq != 0) %>% 
  group_by(Var1, Var2) %>% 
  mutate(
    max_days = max(as.numeric(as.character(Var3))),
    days_supply2 = as.numeric(as.character(Var3))
  ) %>% 
  dplyr::filter(Var3 == max_days) %>% 
  dplyr::ungroup(.) 

ggplot(data = tab4_test4, aes(x = as.factor(Var2), y = days_supply2)) +
  geom_boxplot() + stat_summary(fun.y=mean, geom= "point", shape = 17) +
  ylab("RX_DAYS_SUPPLY") + xlab("GENERIC_MED_NAME") + coord_flip()

tab4_test_wide <- tab4_test4 %>% 
  select(Var1, Var2, days_supply2) %>% 
  spread(., Var2, days_supply2)

tab4_nums <- data.frame(num_obs = unlist(lapply(2:ncol(tab4_test_wide), 
                                         function(x) table(!is.na(tab4_test_wide[[x]]))[2])))

use_vars <- which(!(tab4_nums$num_obs %in% 1))

lapply(2:ncol(tab4_test_wide), function(x) 
  ggplot(data = tab4_test_wide, aes(x = "", y = tab4_test_wide[[x]])) + 
    geom_boxplot() +

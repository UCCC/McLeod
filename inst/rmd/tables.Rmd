...
params: 
    ...
    DBServerName: 
        ...
        input: select
        choices: [cc-s-d05.ucdenver.pvt, ihrsql1.ihr.or.kp.org, PRDVDWSQL01, SQL01, BI01, uat.edwsql.hosp.dhha.org]
    DBName:
        label: "Database Name:"
        value: VDW_3_1_DH_0418
        input: select
        choices: [VDW_3_1_DH, VDW, CHORDS_VDW_CC_3.1, CHORDS_VDW_CT_3.1, CHORDS_VDW_HP_3.1, CHORDS_VDW_MC_3.1, CHORDS_VDW_SC_3.1, CHORDS_VDW_SS_3.1, CHORDSlive.dbo, CHORDSlive, VDW_3_1_DH_0418, VDW_3_1_CHCO_0618, VDW_Aligned]
---
...
# third list of medication names
med_names3 <- surg2rec::med_names3
...
tab1b_sum <- 
    list("Sample size" =
       ...
       "Height (inches)" =
       list("Mean (SD), Male"   = ~ qwraps2::mean_sd(avg_ht[GENDER == "M"], na_rm = T),
            "Mean (SD), Female" = ~ qwraps2::mean_sd(avg_ht[GENDER == "F"], na_rm = T)),
       "Weight (lbs)" = 
       list("Mean (SD), Male"    = ~ qwraps2::mean_sd(avg_wt[GENDER == "M"], na_rm = T),
            "Mean (SD), Female" = ~ qwraps2::mean_sd(avg_wt[GENDER == "F"], na_rm = T)),
...
if(length(dimnames(table(tab1b2$ProcedureType))[[1]]) == 1){
...
length(tab1d$los[tab1d$los < 0])

```

The number of patients with a negative length of stay is `r length(tab1d$los[tab1d$los < 0])`.

```{r out_tab1d, results="asis"}

if(length(dimnames(table(tab1d$ProcedureType))[[1]]) == 1){
...
if(length(dimnames(table(tab1e$ProcedureType))[[1]]) == 1){
  tab1e_tot_sum <- qwraps2::summary_table(dplyr::group_by(tab1e, YEAR), tab1d_sum)
  tab1e_out <- tab1e_tot_sum
  print(tab1e_out)
} else{
  tab1e_tot_sum <- qwraps2::summary_table(tab1e, tab1d_sum)
  ...
  dimnames(tab1e_out)[[2]][1] <- paste0("Total (N = ", nrow(tab1e), ")")
  print(tab1e_out)
}

```

# Figures 1A & B: Histogram of Patient Age at surgery (CPT Code/Proc Code Date) by procedure type

```{r fig1a_data, results="asis"}

if(length(tab1b2$ProcedureType[tab1b2$ProcedureType == "Spine"]) > 0){
  fig1a <- ggplot(data = tab1b2[tab1b2$ProcedureType == "Spine",], aes(x = AGE)) +
  geom_histogram(color="black", fill="red", binwidth = 1) +
  ylab("Count") + xlab("Age") +
  ggtitle("Age histogram for Spine Patients")
  fig1a
} else{
  fig1a <- cat("There were no patients who had a Spine procedure\n")
  fig1a
}

if(length(tab1b2$ProcedureType[tab1b2$ProcedureType == "Pectus"]) > 0){
fig1b <- ggplot(data = tab1b2[tab1b2$ProcedureType == "Pectus",], aes(x = AGE)) +
  geom_histogram(color="black", fill="red", binwidth = 1) +
  ylab("Count") + xlab("Age") +
  ggtitle("Age histogram for Pectus Patients")
  fig1b
} else{
  fig1b <- cat("\nThere were no patients who had a Pectus procedure")
  fig1b
}
...
knitr::kable(tab2a4, col.names = c("Procedure Type", "Primary DX", "Count of DX", "Mean Length of Stay", "Median Length of Stay"), digits = 2)
...
tab2b2 <- tab2b %>% 
  ...
  arrange(PX) %>% 
  mutate(
    dup_px = duplicated(PX) # Mark duplicate PXs
  ) %>% 
  filter(dup_px == FALSE) %>% # Get rid of duplicate PXs
  mutate(
    px_num = paste0("px_", 1:length(PERSON_ID)),
    num_pxs = (1:length(PERSON_ID))
  )

num_pxs_all1 <- max(tab2b2$num_pxs) # Max number of PXs
tab2b2$num_pxs_all <- num_pxs_all1
num_pxs_all2 <- max(tab2b2$num_pxs) - 1

tab2b_wide <- tab2b2 %>% 
  dplyr::select(-num_pxs, -ENC_ID) %>% 
...
table(tab2b4$Freq[tab2b4$ProcedureType == "Pectus"])
...
# Get unique providers for procedures
perf_prov <- unique(tab3a$PERFORMINGPROVIDER)

# Generate code for SQL query of unique providers from above
perf_prov2  <- paste(perf_prov, collapse = "', '")
...
FROM
    ...
    WHERE  z.ADATE >= '01/01/2011'
           AND 
               z.PERFORMINGPROVIDER IN('", perf_prov2, "') 
            AND
               z.PX IN('0RG4070', '0RG4071', '0RG407J', '0RG40A0', '0RG40A1', '0RG40AJ', '0RG40J0', '0RG40J1', '0RG40JJ', '0RG40K0', '0RG40K1', '0RG40KJ', '0RG40Z0', '0RG40Z1', '0RG40ZJ', '0RG6070', '0RG6071', '0RG607J', '0RG60A0', '0RG60A1', '0RG60AJ', '0RG60J0', '0RG60J1', '0RG60JJ', '0RG60K0', '0RG60K1', '0RG60KJ', '0RG60Z0', '0RG60Z1', '0RG60ZJ', '0RG7070', '0RG7071', '0RG707J', '0RG70A0', '0RG70A1', '0RG70AJ', '0RG70J0', '0RG70J1', '0RG70JJ', '0RG70K0', '0RG70K1', '0RG70KJ', '0RG70Z0', '0RG70Z1', '0RG70ZJ', '0RG8070', '0RG8071', '0RG807J', '0RG80A0', '0RG80A1', '0RG80AJ', '0RG80J0', '0RG80J1', '0RG80JJ', '0RG80K0', '0RG80K1', '0RG80KJ', '0RG80Z0', '0RG80Z1', '0RG80ZJ', '0RGA070', '0RGA071', '0RGA07J', '0RGA0A0', '0RGA0A1', '0RGA0AJ', '0RGA0J0', '0RGA0J1', '0RGA0JJ', '0RGA0K0', '0RGA0K1', '0RGA0KJ', '0RGA0Z0', '0RGA0Z1', '0RGA0ZJ', '0SG0070', '0SG0071', '0SG007J', '0SG00A0', '0SG00A1', '0SG00AJ', '0SG00J0', '0SG00J1', '0SG00JJ', '0SG00K0', '0SG00K1', '0SG00KJ', '0SG00Z0', '0SG00Z1', '0SG00ZJ', '0SG1070', '0SG1071', '0SG107J', '0SG10A0', '0SG10A1', '0SG10AJ', '0SG10J0', '0SG10J1', '0SG10JJ', '0SG10K0', '0SG10K1', '0SG10KJ', '0SG10Z0', '0SG10Z1', '0SG10ZJ', '0SG3070', '0SG3071', '0SG307J', '0SG30A0', '0SG30A1', '0SG30AJ', '0SG30J0', '0SG30J1', '0SG30JJ', '0SG30K0', '0SG30K1', '0SG30KJ', '0SG30Z0', '0SG30Z1', '0SG30ZJ', '81', '81.01', '81.02', '81.03', '81.04', '81.05', '81.06', '81.07', '81.08', '81.09', '81.3', '81.35', '81.36', '81.37', '81.38', '81.39', '81.61', '81.62', '81.63', '81.64', '84.51', '0WU80JZ', '0WU84JZ', '34.74', '22532', '22533', '22548', '22551', '22554', '22556', '22558', '22586', '22590', '22595', '22600', '22610', '22612', '22614', '22630', '22633', '22634', '22800', '22802', '22804', '22808', '22810', '22812', '22818', '22819', '22830', '22840', '22841', '22842', '22843', '22844', '22845', '22846', '22847', '22848', '22849', '22850', '22852', '22853', '22854', '22855', '22856', '22857', '22858', '22859', '22861', '22862', '22864', '22865', '22867', '22868', '22869', '22870', '21740', '21742', '21743')
GROUP BY z.ENC_ID,
        z.PERSON_ID,
...
This table contains information only for physicians, not fellows, residents, or other providers. Of the `r n_distinct(tab1b2$PERSON_ID)` total patients, `r n_distinct(tab3a_prov$PERFORMINGPROVIDER)` had provider data that was used to create the table below.

```{r tab3a_out, results="asis"}

knitr::kable(tab3a2_prov, col.names = c("Year", "Provider Specialty", "Cohort Spine PXs", "Total Spine PXs", "Cohort Pectus PXs", "Total Pectus PXs"))
...
Boxplots in the following figures show the median (line in the middle of the box), mean (the + symbol), the 25th and 75th percentiles (outer edges of the box), 1.5 times the interquartile range (IQR; the whiskers), and points beyond 1.5 times the IQR.
...
```{r tab4a_data, fig.height=10, fig.width=15}
...
# Mark medications of interest and recode medications that are similar into same medication name
tab4a2 <- tab4a %>% 
  mutate(
    GENERIC_MED_NAME = stringr::str_to_upper(GENERIC_MED_NAME),
    ...
    med_use3 = if_else(GENERIC_MED_NAME %in% med_names3$GENERIC_MED_NAME, 1, 0),
    GENERIC_MED_NAME = dplyr::recode(GENERIC_MED_NAME, "HYDROCODONE/APAP 5/500" = "HYDROCODONE/ACETAMINOPHEN", "MEPERIDINE HCL/PF" = "MEPERIDINE HCL", "METHADONE HCL ORAL SYRINGE" = "METHADONE HCL", "MORPHINE SULFATE PF" = "MORPHINE SULFATE", "MORPHINE SULFATE SR" = "MORPHINE SULFATE", "MORPHINE SULFATE/PF" = "MORPHINE SULFATE", "OXYCODONE HCL/ACETAMINOPHEN" = "OXYCODONE HCL", "OXYCODONE SA" = "OXYCODONE HCL", "OXYCODONE/APAP 5/325" = "OXYCODONE HCL")
  ) %>% 
  dplyr::filter(med_use3 == 1) %>% 
...
#   geom_boxplot() + stat_summary(fun.y=mean, geom= "point", shape = 17, size = 4) +
...
lapply(5:ncol(tab4_test_wide), function(x) 
  ggplot(data = tab4_test_wide, aes(x = Year, y = tab4_test_wide[[x]], fill = Procedure_Type)) + 
    ...
    geom_jitter(shape = 16, width = 0.2, height = 0) +
...
Number of prescriptions with over 100 RX_DAYS_SUPPLYS = `r ifelse(max(tab4a2$RX_DAYS_SUPPLY > 100) >= 1, table(tab4a2$RX_DAYS_SUPPLY > 100)[[2]], 0)`.  These values were recoded to 100 for scaling .
...
## Table 4D: Prescription supply days by medication and provider specialty

The table below gives the frequency of RX_DAYS_SUPPLY, by GENERIC_MED_NAME, and procedure type. 

```{r tab4d_data, fig.height=10, fig.width=15}

tab4d <- tab4a2 %>% 
  group_by(PERSON_ID, GENERIC_MED_NAME) %>% 
  mutate(
    max_rx_days = max(RX_DAYS_SUPPLY),
    use_days = if_else(RX_DAYS_SUPPLY == max_rx_days, 1, 0)
  ) %>% 
  filter(use_days == 1)

tab4d_sum <- as.data.frame(table(tab4d$ProcedureType, tab4d$SPECIALTY, tab4d$GENERIC_MED_NAME, tab4d$RX_DAYS_SUPPLY))
...
tab4d_test <- tab4d_sum %>% 
  ...
  dplyr::ungroup(.)  %>% 
  select(Procedure_Type = Var1, Specialty = Var2, GENERIC_MED_NAME = Var3, RX_DAYS_SUPPLY = days_supply2, Frequency = Freq)


```

```{r tab4d_out, results="asis"}

knitr::kable(tab4d_test)
...
tab4e_nums <- data.frame(num_obs = unlist(lapply(2:ncol(tab4e_test_wide), 
                                         function(x) table(!is.na(tab4e_test_wide[[x]]))[2])))
...
## Table 4F:  Mean and median age for prescriptions that had refills

```{r tab4f_data, results="asis"}
...
if (nrow(tab4f) == 0){
  ...
  rx_tab <- data.frame(avg_age = unlist(lapply(list_tab4f, 
                                               function(x) mean(as.numeric(tab4f$AGE[tab4f$Procedure_Type == x[[1]] & tab4f$GENERIC_MED_NAME == x[[2]]]), na.rm = T))))
  
  rx_tab2 <- data.frame(med_age = unlist(lapply(list_tab4f, 
                                                function(x) median(as.numeric(tab4f$AGE[tab4f$Procedure_Type == x[[1]] & tab4f$GENERIC_MED_NAME == x[[2]]]), 
                                                                   na.rm = T))))
  
  tab4f2 <- bind_cols(tab4f_vals, rx_tab)
  tab4f3 <- bind_cols(tab4f2, rx_tab2) 
  knitr::kable(tab4f3, col.names = "Procedure Type", "GENERIC_MED_NAME", "Average Age", "Median Age")
}
